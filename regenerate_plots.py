...
# config.DATA_URL may be an absolute path from another machine. Fall back to
...
if not os.path.exists(config.DATA_URL) and os.path.exists(_local_tsv):
    config.DATA_URL = _local_tsv
    import data.loading as _loading
    _loading.DATA_URL = _local_tsv     # the module imported it by value
...
from evaluation.metrics import compute_all_differences, build_summary_df
from visualization import plots


def recompute_ground_truth():
    """Rebuild ground-truth stats from source data (deterministic)."""
...
def main():
    ground_truth_stats = recompute_ground_truth()
    ...
    for stat_type, value_col in [("pearson", "correlation"),
        ...
        plots.plot_rank(stat_type, differences)
